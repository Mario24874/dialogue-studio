 ...
 * Replicates the ItaliantoApp animation: fade+scale → rotate 360° → pulse ×2 → fade out.
...
export default function SplashScreen() {
  ...
  useEffect(() => {
    try {
      const shown = sessionStorage.getItem("ds_splash");
      if (!shown) {
        sessionStorage.setItem("ds_splash", "1");
        setVisible(true);
        // Transition to pulse phase after enter animation (~1.3s)
        setTimeout(() => setPhase("pulse"), 1300);
        // Transition to exit phase after pulse (~800ms)
        setTimeout(() => setPhase("exit"), 2200);
        // Unmount after exit fade (500ms)
        setTimeout(() => setVisible(false), 2750);
      }
    } catch {
      // sessionStorage unavailable — skip splash silently
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {/* Logo — secuencia: fade+scale → rotate 360° → pulse ×2 */}
      <motion.div
      ...
      {/* Nombre de la app — aparece tras el logo */}
      <motion.p
